...
from interface_grafica.interface_grafica_python import tela_acessar_graficos_pressao
from gerar_graficos_pressao import Graficosressao


class AcessarGraficosPressao(QWidget, tela_acessar_graficos_pressao.Ui_Form):
    def __init__(self, nome_documento_completo):
        super().__init__()
        self.setupUi(self)
        self.setWindowTitle('Gráficos - Pressão')

        self.nome_documento_completo = nome_documento_completo

        self.app = QApplication([])

        self.graficos = Graficosressao(self.nome_documento_completo)
